...
def to_sol(new_Nt, old_Nx,sol,rho,u,V):# solution 2D to 1D
    for j in range(0,2*old_Nx):
        for n in range(0,2*new_Nt):
            sol[j*(2*new_Nt+1)+n]=rho[j,n]
            sol[(2*new_Nt+1)*2*old_Nx+j*2*new_Nt+n]=u[j,n]
            sol[(2*2*new_Nt+1)*2*old_Nx+j*(2*new_Nt+1)+n]=V[j,n]
        sol[j*(2*new_Nt+1)+2*new_Nt]=rho[j,2*new_Nt]
        sol[(2*2*new_Nt+1)*2*old_Nx+j*(2*new_Nt+1)+2*new_Nt]=V[j,2*new_Nt]
    return 0
...
def initialguess(snes, X):
    ...
    new1_rho=np.zeros((2*old_Nx,old_Nt+1))
    new1_u=np.zeros((2*old_Nx,old_Nt))
    new1_V=np.zeros((2*old_Nx,old_Nt+1))
    for n in range(old_Nt):
        new1_rho[:,n]=interpol(old_Nx,2*old_Nx,rho[:,n])
        new1_u[:,n]=interpol(old_Nx,2*old_Nx,u[:,n])
        new1_V[:,n]=interpol(old_Nx,2*old_Nx,V[:,n])
    new1_rho[:,old_Nt]=interpol(old_Nx,2*old_Nx,rho[:,old_Nt])
    new1_V[:,old_Nt]=interpol(old_Nx,2*old_Nx,V[:,old_Nt])
    new_rho=np.zeros((2*old_Nx,2*new_Nt+1))
    new_u=np.zeros((2*old_Nx,2*new_Nt))
    new_V=np.zeros((2*old_Nx,2*new_Nt+1))
    for j in range(2*old_Nx):
    ...
    new_w = np.zeros(3*(2*new_Nt)*(2*old_Nx)+2*(2*old_Nx))
