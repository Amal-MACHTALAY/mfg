...
def compute_jacobian(w:'float[:]', row:'int[:]', col:'int[:]', data:'float[:]', 
    ...
    for n in range(ranges[0][0], ranges[0][1]):
            row[cmpt] = Fr_idx(Nx,n,Nt); col[cmpt] = r_idx(Nx,n+1,Nt); data[cmpt] = 1
            cmpt +=1
            row[cmpt] = Fu_idx(Nx,n,Nt,Nx); col[cmpt] = u_idx(Nx,n,Nt,Nx); data[cmpt] = 1
            cmpt +=1
            row[cmpt] = FV_idx(Nx,n,Nt,Nx); col[cmpt] = V_idx(Nx,n,Nt,Nx); data[cmpt] = -1
            cmpt +=1
            row[cmpt] = FV_idx(Nx,n,Nt,Nx); col[cmpt] = V_idx(Nx,n+1,Nt,Nx); data[cmpt] = 1-2*eps
            cmpt +=1
            
            row[cmpt] = Fr_idx(Nx,n,Nt); col[cmpt] = r_idx(1,n,Nt); data[cmpt] = (0.5*dt/dx)*w[u_idx(1,n,Nt,Nx)]-0.5
            cmpt +=1
            row[cmpt] = Fr_idx(Nx,n,Nt); col[cmpt] = u_idx(1,n,Nt,Nx); data[cmpt] = (0.5*dt/dx)*w[r_idx(1,n,Nt)]
            cmpt +=1
...
def compute_FF(w:'float[:]', FF:'float[:]', Nt:'int', Nx:'int', dt:'float', dx:'float', eps:'float',
    ...
    for n in range(ranges[0][0], ranges[0][1]):
        ...
        if j==1:
            JJ=j-ranges[1][0]
            # F_rho , F[0]->F[Nt-1] ************** 1  
            FF[Fr_idx(JJ,NN,Ntloc)]=w[r_idx(1,n+1,Nt)]-0.5*(w[r_idx(Nx,n,Nt)]+w[r_idx(2,n,Nt)])\
                +(0.5*dt/dx)*(w[r_idx(2,n,Nt)]*w[u_idx(2,n,Nt,Nx)]-w[r_idx(Nx,n,Nt)]*w[u_idx(Nx,n,Nt,Nx)])
            # F_u , F[Nt*Nx]->F[Nt*Nx+Nt-1] *********** 4 
            FF[Fu_idx(JJ,NN,Ntloc,Nxloc)]=w[u_idx(1,n,Nt,Nx)]-f_star_p((w[V_idx(1,n+1,Nt,Nx)]-w[V_idx(Nx,n+1,Nt,Nx)])/dx,w[r_idx(1,n,Nt)], u_max, rho_jam )
            # F_V , F[2*Nt*Nx]->F[2*Nt*Nx+Nt-1] *********** 7 
            FF[FV_idx(JJ,NN,Ntloc,Nxloc)]=w[V_idx(1,n+1,Nt,Nx)]-w[V_idx(1,n,Nt,Nx)]\
        ...
        if j==Nx:
            ...
            FF[Fr_idx(JJ,NN,Ntloc)]=w[r_idx(Nx,n+1,Nt)]-0.5*(w[r_idx(Nx-1,n,Nt)]+w[r_idx(1,n,Nt)])\
                +(0.5*dt/dx)*(w[r_idx(1,n,Nt)]*w[u_idx(1,n,Nt,Nx)]-w[r_idx(Nx-1,n,Nt)]*w[u_idx(Nx-1,n,Nt,Nx)])
            # F_u , F[2*Nt*Nx-Nt]->F[2*Nt*Nx-1] ********* 6 
            FF[Fu_idx(JJ,NN,Ntloc,Nxloc)]=w[u_idx(Nx,n,Nt,Nx)]-f_star_p((w[V_idx(Nx,n+1,Nt,Nx)]-w[V_idx(Nx-1,n+1,Nt,Nx)])/dx,w[r_idx(Nx,n,Nt)], u_max, rho_jam)
            # F_V , F[3*Nt*Nx-Nt]->F[3*Nt*Nx-1] ********** 9 
            FF[FV_idx(JJ,NN,Ntloc,Nxloc)]=w[V_idx(Nx,n+1,Nt,Nx)]-w[V_idx(Nx,n,Nt,Nx)]\
    ...
    for j in range(ranges[1][0], ranges[1][1]): # 2,Nx
        ...
        if j != 0 and j != 1:
            ...
            FF[Frint_idx(JJ,Ntloc,Nxloc)]=w[r_idx(j,0,Nt)]-(1/dx)*integrate_rho_int_v2(x[j-1],x[j])
            # F_V_ter , F[3*Nt*Nx+Nx+1]->F[3*Nt*Nx+2*Nx-2] ********* 14
            FF[FVter_idx(JJ,Ntloc,Nxloc)]=w[V_idx(j,Nt,Nt,Nx)]-VT(x[j])
    ...
    if j==1:
        ...
        FF[Frint_idx(JJ,Ntloc,Nxloc)]=w[r_idx(1,0,Nt)]-(1/dx)*integrate_rho_int_v2(x[0],x[1])
        # F_V_ter , F[3*Nt*Nx+Nx] *********** 13 
        FF[FVter_idx(JJ,Ntloc,Nxloc)]=w[V_idx(1,Nt,Nt,Nx)]-VT(x[1])
    j=ranges[1][1]
    if j==Nx:
        JJ=j-ranges[1][0]
        # F_rho_int , F[3*Nt*Nx+Nx-1] ********* 12
        FF[Frint_idx(JJ,Ntloc,Nxloc)]=w[r_idx(Nx,0,Nt)]-(1/dx)*integrate_rho_int_v2(x[Nx-1],x[Nx])
        # F_V_ter , F[3*Nt*Nx+2*Nx-1] ************** 15
        FF[FVter_idx(JJ,Ntloc,Nxloc)]=w[V_idx(Nx,Nt,Nt,Nx)]-VT(x[Nx])
