...
def U_2LWR(u_max,rho1,rho2): # u_max= u1_max or u2_max
    return u_max*(1-(rho1*l1+rho2*l2))

# Cost functional
def f_mfg_2LWR(u,u_max,rho1,rho2): # u= u1 or u2
    return 0.5*((U_2LWR(u_max,rho1,rho2)-u)**2) # 2-MFG-LWR

def f_star_p_2LWR(u_max,p,rho1,rho2): # # u_max= u1_max or u2_max
    return U_2LWR(u_max,rho1,rho2)-p 
    
def f_star_2LWR(u_max,p,rho1,rho2): # p=Vx
    return -0.5*(p**2)+U_2LWR(u_max,rho1,rho2)*p 
...
def Fct(w,f_star_p,f_star):
    # FF=[F_rho,F_u,F_V,F_rho_int,F_V_ter], F_rho:0->Nt*2*Nx-1, F_u:Nt*2*Nx->2*Nt*2*Nx-1, F_V:2*Nt*2*Nx->3*Nt*2*Nx-1, 
    # F_rho_int:3*Nt*2*Nx->3*Nt*2*Nx+2*Nx-1, F_V_ter:3*Nt*2*Nx+2*Nx->3*Nt*2*Nx+2*2*Nx-1
    ...
    for n in range(0,Nt):
        ...
        FF[FV_idx(1,n)]=w[V_idx(1,n+1)]-w[V_idx(1,n)]+dt*f_star(u1_max,(w[V_idx(1,n+1)]-w[V_idx(Nx,n+1)])/dx,w[r_idx(1,n)],w[r_idx(Nx+1,n)])\
            +eps*(w[V_idx(2,n+1)]-2*w[V_idx(1,n+1)]+w[V_idx(Nx,n+1)])
        # F_V2 , F[5*Nt*Nx]->F[5*Nt*Nx+Nt-1] *********** 16
        FF[FV_idx(Nx+1,n)]=w[V_idx(Nx+1,n+1)]-w[V_idx(Nx+1,n)]+dt*f_star(u2_max,(w[V_idx(Nx+1,n+1)]-w[V_idx(Nx+Nx,n+1)])/dx,w[r_idx(1,n)],w[r_idx(Nx+1,n)])\
            +eps*(w[V_idx(Nx+2,n+1)]-2*w[V_idx(Nx+1,n+1)]+w[V_idx(Nx+Nx,n+1)])
        # F_V1 , F[5*Nt*Nx-Nt]->F[5*Nt*Nx-1] ********** 15 
        FF[FV_idx(Nx,n)]=w[V_idx(Nx,n+1)]-w[V_idx(Nx,n)]+dt*f_star(u1_max,(w[V_idx(Nx,n+1)]-w[V_idx(Nx-1,n+1)])/dx,w[r_idx(Nx,n)],w[r_idx(Nx+Nx,n)])\
            +eps*(w[V_idx(1,n+1)]-2*w[V_idx(Nx,n+1)]+w[V_idx(Nx-1,n+1)])
        # F_V2 , F[6*Nt*Nx-Nt]->F[6*Nt*Nx-1] ********** 18 
        FF[FV_idx(Nx+Nx,n)]=w[V_idx(Nx+Nx,n+1)]-w[V_idx(Nx+Nx,n)]+dt*f_star(u2_max,(w[V_idx(Nx+Nx,n+1)]-w[V_idx(Nx+Nx-1,n+1)])/dx,w[r_idx(Nx,n)],w[r_idx(Nx+Nx,n)])\
    ...
    for j in range(2,Nx):
        for n in range(0,Nt):
            ...
            FF[FV_idx(j,n)]=w[V_idx(j,n+1)]-w[V_idx(j,n)]+dt*f_star(u1_max,(w[V_idx(j,n+1)]-w[V_idx(j-1,n+1)])/dx,w[r_idx(j,n)],w[r_idx(Nx+j,n)])\
                +eps*(w[V_idx(j+1,n+1)]-2*w[V_idx(j,n+1)]+w[V_idx(j-1,n+1)])
            # F_V2 , F[5*Nt*Nx+Nt]->F[6*Nt*Nx-Nt-1] ********* 17 
            FF[FV_idx(Nx+j,n)]=w[V_idx(Nx+j,n+1)]-w[V_idx(Nx+j,n)]+dt*f_star(u2_max,(w[V_idx(Nx+j,n+1)]-w[V_idx(Nx+j-1,n+1)])/dx,w[r_idx(j,n)],w[r_idx(Nx+j,n)])\
...
# F  = lambda x : Fct(x,f_star_p_2LWR,f_star_2LWR)
# xglo = None
...
for i in range(0,5):
    ...
    if mu==0.0:
        ...
        dt=min(T/Nt,CFL*dx/max(u1_max,u2_max),EPS*(dx**2)/mu) # temporal step size
        eps=mu*dt/(dx**2) # V
